...
impl NormalNoise {
    pub fn create(random: &mut WorldRandom, first_octave: i32, amplitudes: Vec<f64>) -> Self {
        ...
        let span = match (min_octave, max_octave) {
            (Some(lo), Some(hi)) => (hi - lo + 1) as f64,
            _ => 1.0,
        };
        // PARITY-CHECK: reconstructed from memory of `NormalNoise`'s constructor — the
        // `10.0 / 6.0` factor and its division by the active-octave span are the
        // least-certain constant in this crate. Must be checked against decompiled 26.2
        // source (or Java test vectors) before trusting sampled `minecraft:noise` values.
        let value_factor = (10.0 / 6.0) / span;
