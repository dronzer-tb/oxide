...
| 3 | `oxide-biome`, `oxide-chunkgen` | 1 | done (scoped) — `b3f5671`, 9 tests. Terrain fill + heightmaps + biome grid. **Surface rules implemented 2026-08-23** (`surface.rs`, all 15 condition types, verified against the real 26.2 pack: land columns come out `grass_block`/`dirt`, not bare stone) with two named gaps — `bandlands` banding and the temperature-noise half of `minecraft:temperature`. Aquifers/ore veins/carvers still deferred, see `fill.rs` |
...
| 5 | `plugin/` router, `oxide-ffi` | 5, 6, 7 | done (scoped) — 5 Rust tests + Java compiles (CI-verified, not yet run against a live Folia server). `/oxide createworld` and, since 2026-08-23, `getDefaultWorldGenerator` (per-world opt-in via `bukkit.yml`) hook Bukkit's `ChunkGenerator` to `oxide-chunkgen` over Panama. The FFI carries palette indices, so real block variety and the biome grid cross the boundary; `OxideBiomeProvider` answers per-position biome queries. Native lib is embedded in the jar by CI, linux-x86_64 only. No caves/aquifers/ore veins; structures and decoration are left to vanilla |
